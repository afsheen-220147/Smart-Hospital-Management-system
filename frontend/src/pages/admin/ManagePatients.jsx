...
import { User, Activity, Trash2, Calendar, FileText, Search, Loader2, AlertCircle, RefreshCw, Clock, CheckCircle } from 'lucide-react'
...
export default function ManagePatients() {
  ...
  const [approvalModal, setApprovalModal] = useState(null) // Show approval status modal
  ...
  useEffect(() => { 
    ...
    // Auto-refresh patient data every 20 seconds for dynamic visit history updates
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // Show approval modal with action details
      setApprovalModal({
        actionId: response.data.actionId,
        type: 'patient',
        approvals: response.data.approvals || 1,
        approvalsNeeded: response.data.approvalsNeeded || 3
      })
      
      showSuccess('Deletion request initiated! Waiting for admin approvals...')
      setDeleteId(null)
      ...
      {approvalModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 border border-gray-100">
            ...
            <h2 className="text-2xl font-bold text-center mb-2">Deletion Requested</h2>
            <p className="text-gray-600 text-center mb-6">Waiting for admin approvals</p>
            
            <div className="space-y-4 mb-6">
              <div className="bg-gray-50 p-4 rounded-xl">
                <p className="text-xs text-gray-500 mb-1">Action ID</p>
                <p className="font-mono text-sm font-semibold text-gray-900 break-all">{approvalModal.actionId}</p>
              </div>
              
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Approvals Required:</span>
                <span className="text-xl font-bold">
                  <span className="text-amber-600">{approvalModal.approvals}</span>
                  <span className="text-gray-400">/{approvalModal.approvalsNeeded}</span>
                </span>
              </div>
              ...
              <div className="relative h-2 bg-gray-200 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${(approvalModal.approvals / approvalModal.approvalsNeeded) * 100}%` }}
                />
              </div>
              
              <p className="text-xs text-gray-500 text-center">
                {approvalModal.approvalsNeeded - approvalModal.approvals} more approvals needed
              </p>
            </div>
